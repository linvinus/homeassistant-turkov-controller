...
class TurkovControllerStateProxy:
    ...
    async def async_update(self, event_time):
        ...
        try:
            ...
            _LOGGER.error("[" + sys._getframe().f_code.co_name + "] async_update: %s", resp)
            json_response = await resp.json(content_type=None)
            _LOGGER.error("[" + sys._getframe().f_code.co_name + "] async_update: %s", json_response)
    ...
    async def set_on(self, new_state) -> bool:
        ...
        try:
            websession = async_get_clientsession(self._hass)
            with async_timeout.timeout(REQUEST_TIMEOUT):
                resp = await websession.post("http://%s/command" % (self._host),json={"on":str(new_state).lower()})
            if resp.status != 200:
                _LOGGER.error(f"{resp.url} returned {resp.status}")
                return
